...
**Aktuální verze:** 1.16.24  
**Platforma:** GHRAB Platform 1.1.2


Dílna výukových her pro interní školní použití Gymnázia, Ostrava-Hrabůvka. Verze 1.16.24 je patch kandidát navazující na auditovanou GARP 2.5.1 baseline 1.16.23; opravuje kontrakt publikovaného Studio manifestu pro automatickou patch promotion AI Studia.

## Aktuální stav

Verze: **1.16.24**

LUDUS 1.16.24 nemění pedagogickou logiku, enginy, AI prompty ani bezpečnostní politiku. Opravuje postprocessing `studio-manifest.json`, aby výsledný nasazený manifest zachoval canonical GHRAB Platform pole vyžadovaná AI Studiem. Tím připravuje reálný test automatického převzetí patch verze 1.16.23 → 1.16.24. Bezpečnostní baseline GARP 2.5.1 zůstává 1.16.23 a pro 1.16.24 se znovu spouštějí lokální GARP/QA brány; historické reporty 1.16.23 se nepřepisují. Do samostatného schválení LIVE provozu nadále platí omezení na syntetická data.
...
Při práci s obsahem je nutná anonymizace. Do AI generování se nevkládají jména žáků, individuální hodnocení ani jiné osobní či citlivé údaje. Kandidát 1.16.24 má navíc explicitní omezení: **REÁLNÁ STUDENTSKÁ DATA: NEPOUŽÍVAT** a **TESTOVACÍ PROVOZ POUZE SE SYNTETICKÝMI DATY**, dokud GARP gate nebudou uzavřeny.
...
LUDUS 1.16.24 podporuje Studio Bridge v2 s kompatibilitou v1 a ruční import `ghrab-material-v1` / `LUDUS_CONTENT v2`. Podporované strukturované úlohy převádí na stanice a připraví engine, třídní soutěž nebo lesson pack bez dalšího AI volání.
