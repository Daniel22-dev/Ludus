...
try {
  ...
  const x=readJson(file), p=readJson(a.profile), errors=[]; let structuralFail=false;
  ...
  for(const req of p.requiredComponents||[]){
    if(req.required!==true)continue; const c=by.get(req.componentId);
    ...
    if(!good){if(req.serverDependent&&p.serverApproval!=='APPROVED'&&['NOT_TESTED','UNKNOWN','MISSING'].includes(c.effectiveness))liveNotTested=true;else{localFail=true;errors.push(`${req.componentId}: failing assurance axes`);}continue;}
    ...
    for(const ref of c.evidenceRefs){if(!isObject(ref)||!ref.id||!SHA256_RE.test(String(ref.sha256||''))){errors.push(`${req.componentId}: invalid evidence ref`);localFail=true;continue;}const trusted=p.trustedEvidence?.[ref.id];if(!trusted||String(trusted).toLowerCase()!==String(ref.sha256).toLowerCase()){errors.push(`${req.componentId}: untrusted evidence ${ref.id}`);localFail=true;}}
  }
