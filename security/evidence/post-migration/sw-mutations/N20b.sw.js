...
const LUDUS_CACHE = "ghrab-ludus-v1.16.22";
...
self.addEventListener('fetch', (event) => {
  ...
  if (request.destination === 'script') { event.respondWith(caches.match(request)); return; }
...
const GHRAB_PLATFORM_P3_ASSETS=["./ghrab/ghrab-platform.css","./ghrab/ghrab-artifact-envelope-v1.schema.json","./ghrab/ghrab-app-registry-v2.schema.json","./ghrab/ghrab-platform-manifest-1.1.2.json","./assets/brand/school-logo.png"];
self.addEventListener('install',event=>event.waitUntil((async()=>{const cache=await caches.open("ghrab-ludus-v1.16.22");const results=await Promise.allSettled(GHRAB_PLATFORM_P3_ASSETS.map(asset=>cache.add(asset)));const failed=results.filter(item=>item.status==='rejected');if(failed.length)throw new Error('GHRAB Platform P3 precache selhal: '+failed.length);})()));
