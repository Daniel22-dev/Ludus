...
const iteratorCoveredWrites = new Set();
const reEscape = value => String(value).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
function addWritePositions(segment, offset, varName) {
  const re = new RegExp(`\\.\\s*add\\s*\\(\\s*${reEscape(varName)}\\s*\\)`, 'g');
  return [...segment.matchAll(re)].map(m => offset + m.index);
}
function registerIteratorSite({ receiver, varName, body, bodyOffset, source, pos }) {
  const writes = addWritePositions(body, bodyOffset, varName);
  if (!writes.length) return false;
  for (const idx of writes) iteratorCoveredWrites.add(idx);
  const values = resolveExpr(receiver);
  if (values) addResolved(values, `${source}:${receiver.trim()}`);
  else noteUnresolved(pos, 'add', receiver, source);
  return true;
}

// for..of iterator paths. A write is associated with the exact lexical loop site,
// never with a variable name reused elsewhere in the file.
for (const m of sw.matchAll(/\bfor\s*\(\s*(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s+of\s+Object\.values\s*\(\s*([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*)\s*\)\s*\)\s*\{/g)) {
  ...
  if (endBrace < 0) { noteUnresolved(m.index, 'add', `Object.values(${m[2]})`, 'for-of-unbalanced'); continue; }
  const bodyOffset = openBrace + 1, body = sw.slice(bodyOffset, endBrace);
  registerIteratorSite({ receiver: `Object.values(${m[2]})`, varName: m[1], body, bodyOffset, source: 'for-of-add', pos: m.index });
}
for (const m of sw.matchAll(/\bfor\s*\(\s*(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s+of\s+([^)]*)\)\s*\{/g)) {
  if (/^\s*Object\.values\s*\(/.test(m[2])) continue; // handled by the nested-parenthesis parser above
  const openBrace = m.index + m[0].lastIndexOf('{'), endBrace = scanBalanced(sw, openBrace, '{', '}');
  if (endBrace < 0) { noteUnresolved(m.index, 'add', m[2], 'for-of-unbalanced'); continue; }
  const bodyOffset = openBrace + 1, body = sw.slice(bodyOffset, endBrace);
  registerIteratorSite({ receiver: m[2], varName: m[1], body, bodyOffset, source: 'for-of-add', pos: m.index });
}

// map/forEach iterator paths. Support both `asset =>` and `(asset) =>` and bind the
// callback parameter to this exact call site. This closes LU-N2/P4 name-collision bypasses.
const iteratorCallRe = /((?:[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*)|(?:Object\.values\s*\([^)]*\)))\.(map|forEach)\s*\(/g;
for (const m of sw.matchAll(iteratorCallRe)) {
  const open = m.index + m[0].lastIndexOf('('), close = scanBalanced(sw, open);
  if (close < 0) { noteUnresolved(m.index, 'add', m[1], `${m[2]}-unbalanced`); continue; }
  const innerOffset = open + 1, inner = sw.slice(innerOffset, close);
  const cb = inner.match(/^\s*(?:\(\s*([A-Za-z_$][\w$]*)\s*\)|([A-Za-z_$][\w$]*))\s*=>/s);
  if (!cb) {
    if (/\.\s*add\s*\(/.test(inner)) noteUnresolved(m.index, 'add', m[1], `${m[2]}-callback-unparsed`);
    continue;
  }
  const varName = cb[1] || cb[2];
  const bodyOffset = innerOffset + cb[0].length, body = inner.slice(cb[0].length);
  registerIteratorSite({ receiver: m[1], varName, body, bodyOffset, source: `${m[2]}-add`, pos: m.index });
}

// Direct Cache API writes. Every write must either resolve directly, be tied to a
// resolved iterator call site above, or be explicitly recognized as a bounded runtime helper.
for (const m of sw.matchAll(/\.\s*(addAll|add|put)\s*\(/g)) {
  ...
  if (values) addResolved(values, `${method}:expression`);
    ...
    if (method === 'add' && iteratorCoveredWrites.has(m.index)) {
      // exact lexical iterator site already resolved above
