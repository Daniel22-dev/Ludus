...
async function candidatePool(){
  const abs=[];
  // LU-N17: executable AI/egress paths can live outside src/. Cover engine/runtime/public/script roots.
  for(const rel of ['src','engines','runtime','public','scripts']) abs.push(...await walk(path.join(root,rel)));
  for(const rel of ['public/ai-operations.json','ghrab-ai-core.consumer.json','scripts/build-school-profile.mjs']){
...
const result={status:errors.length?'FAIL':'PASS',errors,files:rows.length,aggregate,expected:fingerprint.aggregate,inventory:path.resolve(inventoryArg),detectedCandidates:detectedUnique.length,untracked,inventoryEntriesNotDetectorMatched:inventoryMissingCandidates,verifierRevision:'garp-2.5.1-lu-n17-ai-inventory-drift-hotfix'};
