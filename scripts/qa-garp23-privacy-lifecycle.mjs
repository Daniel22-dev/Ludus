...
const report={schema:'ghrab-garp23-privacy-lifecycle-v1',appId:'ludus',appVersion:'1.16.23',mode:'deterministic-browser-storage-equivalent',syntheticCanaries:{A,B},checks,total:checks.length,passed:checks.length-failures.length,failed:failures.length,evidence,status:failures.length?'failed':'passed',limitations:['This Platform 1.1.2 wave environment cannot navigate local HTTP because Chromium has managed URLBlocklist=["*"].','Independent Claude review must repeat A/B/new-tab/restart/multi-tab/export-open in an unblocked real browser.']};
